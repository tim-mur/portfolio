// ── Фразы фотографа ───────────────────────────────────────
// Переопределяем PHRASES сразу после загрузки phrase.js,
// до того как init() вызовет startTypewriter().

PHRASES.ru = [
  'НЕ ПРОСТО КНОПКУ НАЖИМАЮ',
  'ХЕЙТЕР ЗОЛОТОГО СЕЧЕНИЯ',
  'ШАБЛОНОНЕНАВИСТНИК'
];
...
(function () {
  ...
  // ── Синхронизация ширины категорий с заголовком ───────
...
// ── Автовыбор режима "фотограф" после intro ───────────────
// phrase.js ставит _introFinished = true внутри onFinish,
// после чего switchMode безопасно меняет UI без поломки анимаций.

(function () {
  var check = setInterval(function () {
    if (typeof _introFinished !== 'undefined' && _introFinished) {
      clearInterval(check);
      if (typeof switchMode === 'function') switchMode('photographer');
    }
  }, 30);
})();


// ── Навигация между страницами через mode-switcher ────────
// designer  → designer.html
// photographer → остаёмся (уже здесь)

document.addEventListener('DOMContentLoaded', function () {
  var btnD = document.getElementById('mode-designer');
  if (btnD) {
    btnD.addEventListener('click', function () {
      // Даём анимации sweep успеть запуститься (450ms в phrase.js), потом переходим
      setTimeout(function () {
        window.location.href = '/designer/index.html?mode=designer';
      }, 300);
    });
  }
});